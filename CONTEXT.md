...
19. **Disk-cached PREPOST** via `prepare_pre_post_harmonization_cached(cfg)` in `eda_core.py`. Hash key covers EDAConfig fields + CSV/HVG mtime+size; first call computes (slow ComBat fit), subsequent calls load in seconds. Cache lives at `notebooks/cache/prepost/<hash>.pkl`. Same cache pattern at `notebooks/cache/genewise_kendall/<hash>.pkl` for the Kendall long table. **Out is reserved for true prediction artifacts; helper-level caches live under `notebooks/cache/`.**
...
27. **Eligibility ordering invariant**: `build_subject_eligibility` runs *before* `apply_gtex_ahba_matching_policy` in both `loro_cache.load_dataset` and `_load_expression`. Eligibility = "did the subject sample ≥`min_observed_parcels` distinct anatomical regions?" — a property of the data, not the bucketing. Without this ordering, `centroids_and_volumes` artificially dropped 5 subjects who had `brain - cerebellum` + `brain - cerebellar hemisphere` (policy collapse → 4 distinct parcels → fails threshold). New caches must keep this ordering; PREPOST and the cache builder must agree on the eligible set.
...
- eligibility ordering (`build_subject_eligibility` *before* `apply_gtex_ahba_matching_policy`) — see Key Recent Change #27. Reordering re-introduces silent subject loss under any non-`centroids` policy.
