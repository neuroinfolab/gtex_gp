...
- **`prepare_pre_post_harmonization_cached(cfg, *, force_rebuild=False, cache_dir=None)`** — disk-cached PREPOST. Hash key covers EDAConfig data-lineage fields (`csv_path`, `cache_root`, `gene_scope`, `min_observed_parcels`, `combat_use_covariates`, `gtex_rep_mode`, `gtex_hemi_mode`) + resolved `matching_policy` + the CSV file's `(mtime_ns, size)`. `hvg_path` is no longer in the dataclass — the HVG gene panel for `gene_scope='hvg'` is sourced from the LORO cache's subject npz `gene_names`, removing one source of cache-key churn. Default cache: `notebooks/cache/prepost/<hash>.pkl`. Defensive chmod-after-mkdir + warn-and-continue on PermissionError covers the historical NFSv4-ACL-zero-mode bug.
...
- **Eligibility ordering invariant (revised)**: After diagnostics established that the prior eligibility-before-policy ordering was admitting subjects with sub-threshold post-policy coverage (LORO trains on k-1 covered parcels), the ordering was reversed. `apply_gtex_ahba_matching_policy` now runs *before* `build_subject_eligibility` in both `loro_cache.load_dataset` and `_load_expression`. `min_observed_parcels` is now a post-policy floor — the active policy IS the analysis bucketing, so eligibility is measured under it. Empirical impact: `centroids` floor=5 → 318 eligible; `centroids_and_volumes` floor=5 → 313 eligible (5-subject delta = the boundary subjects with both cerebellum forms); floor=4 admits 17 additional sparse-sampled subjects under either policy.
- **Dynamic sbatch sizing**: `run_loro_cache_array.sbatch` uses `--array=1-385%64` (full GTEx pool ceiling). `_pick_subjects` in `scripts/run_loro_cache_batch.py` exits cleanly with a `[skip-out-of-range]` log line when `task_id - 1 >= n_eligible`. The same submission works for any `MATCHING_POLICY` × `MIN_OBSERVED_PARCELS` configuration without manual tuning.
...
- Subject eligibility is computed on the **post-policy** `parcel_idx`, i.e. *after* `apply_gtex_ahba_matching_policy`. The active policy is the analysis bucketing, so `min_observed_parcels` is a post-collapse floor: subjects below it never enter the cache and PREPOST filters them identically. Both `loro_cache.load_dataset` and `_load_expression` must keep policy-then-eligibility ordering or the eligible sets diverge silently.
- EDAConfig minimal surface is `(csv_path, cache_root, gene_scope)`; these three (plus `min_observed_parcels` and harmonization knobs) are the data-lineage keys that drive PREPOST / kendall / eval-table caches. `hvg_path` is intentionally absent — HVG panels are sourced from cache npz when `gene_scope='hvg'`. New top-level cfg fields should be justified by an entry in this section.
- Notebook CFG cells follow the same minimal pattern; eval notebooks carry a Table-of-Contents markdown right after the title for navigation. Section sublist defaults to `'gtex_100hvg'` rather than `'ahba_100hvg'` (lists ranked from GTEx-side variance, the more relevant signal for cross-dataset alignment work).
