...
from ._utils import fit_genewise_affine_on_overlap, safe_std


@dataclass
class ZScoreAffineHarmonizer:
    genes: List[str]
    ahba_loc: np.ndarray
    ...
    @classmethod
    def fit(cls, ahba_df: pd.DataFrame, gtex_df: pd.DataFrame, gene_cols: List[str]) -> "ZScoreAffineHarmonizer":
