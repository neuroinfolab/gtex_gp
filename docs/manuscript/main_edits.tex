...
\cite{johnson2007combat}.  For each gene, the pooled expression table is first
regressed on row-level covariates.  By default these covariates include age, sex,
and broad anatomical macro-system; the macro-system term can be omitted in ablation
runs.  The fitted covariate contribution is removed before estimating platform
effects, so that biological and anatomical structure is not treated as batch
variation.

After covariate removal, each gene is standardized using pooled mean and variance.
Within this standardized space, the AHBA and GTEx batches receive gene-wise additive
and multiplicative corrections.  The batch mean and variance estimates are shrunk
toward batch-wide averages across genes to stabilize estimates:
\[
  s_{ig} = \frac{x_{ig} - c_i^\top \hat{\beta}_g - \hat{\mu}_g}{\hat{\sigma}_g},
  \qquad
  s^{adj}_{ig}
  =
  \frac{s_{ig} - \hat{\gamma}^{*}_{b(i),g}}
       {\sqrt{\hat{\delta}^{*}_{b(i),g}}}.
\]
The corrected expression is then reconstructed by restoring the pooled gene scale
and the fitted covariate contribution:
\[
  x^{corr}_{ig}
  =
  s^{adj}_{ig}\hat{\sigma}_g + \hat{\mu}_g + c_i^\top \hat{\beta}_g .
\]

Finally, the implementation applies a post-ComBat calibration from corrected GTEx
to corrected AHBA on the shared parcel support.  For each gene, parcel-level AHBA
means are regressed on parcel-level GTEx means over overlapping parcels,
\[
  \bar{x}^{corr}_{A,rg}
  \approx
  a_g \bar{x}^{corr}_{G,rg} + d_g ,
\]
and the fitted gene-wise affine map is applied to GTEx rows.  Thus AHBA harmonized
values are the corrected AHBA values, while GTEx harmonized values are the corrected
GTEx values after this overlap-based affine calibration.  PLAM adds a further
subject-level calibration layer after this shared preprocessing step
