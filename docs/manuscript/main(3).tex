...
\subsection{Implementation via ComBat}
\label{sec:harmonization-combat}

In our implementation, the harmonization functions in Eq.~\eqref{eq:shared-harmonization}
are realized by the ComBat batch-correction procedure
\cite{johnson2007combat,bolstad2003normalization}.
ComBat models the observed expression of gene $g$ in platform (batch) $b$ as
\begin{equation}
  \label{eq:combat}
  x_{g}^{(b)} = \mu_g + \gamma_{b,g} + \delta_{b,g}\,x_g^{(*)} + \varepsilon_g,
\end{equation}
where $\mu_g$ is the overall gene mean, $\gamma_{b,g}$ is an additive batch shift,
$\delta_{b,g}$ is a multiplicative batch scale, $x_g^{(*)}$ is the underlying
platform-free expression, and $\varepsilon_g \sim \mathcal{N}(0,\sigma_g^2)$.
The batch parameters $(\gamma_{b,g}, \delta_{b,g})$ are estimated by empirical Bayes,
pooling information across genes to stabilize estimates for genes with few observations
\cite{johnson2007combat}.  Harmonized expression is obtained by removing the estimated
batch effects:
\[
  \mathcal{H}_b(x_g^{(b)};\,\hat\gamma,\hat\delta)
  = \frac{x_g^{(b)} - \hat\gamma_{b,g}}{\hat\delta_{b,g}}.
\]
Quantile normalization is applied prior to ComBat to equalize within-platform gene
distributions before cross-platform correction \cite{bolstad2003normalization}.

PLAM adds one further calibration layer after this shared
preprocessing step (Section~\ref{sec:calibration}): gene-wise subject-specific
coefficients $(a_{s,g}, b_{s,g})$ adjust residual within-subject mismatch inside
the latent observation model rather than replacing the global harmonizer.
...
\section{Multi-Subject Spatial Gene Maps}
Cohort-wide hemisphere maps make it possible to compare how each method propagates
sparse subject observations over unobserved parcels for individual genes.  Figure~\ref{fig:spatial-top3-hvg}
shows three high-variance genes selected because they provide the clearest visual
contrast between the learned alignment models and the naive atlas-fill baseline.
Across these examples, DLAM and PLAM preserve localized low-expression structure
that is attenuated or overwritten by naive fill, while also differing subtly from
one another in subject-specific ways.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figs/spatial_top3_hvg_panel.pdf}
\caption{Cohort-wide spatial comparison for three selected high-variance genes.
Panels (A)--(C) show \textit{SLC17A7}, \textit{NRGN}, and \textit{ITPKA}.  Within each
panel, the top row gives the AHBA atlas reference map for that gene and the remaining
rows show the first subject block from the full multi-subject hemisphere-map diagnostic,
comparing sparse GTEx observations, DLAM, PLAM, and naive fill in harmonized expression
space.  These genes were selected because they exhibit the clearest qualitative
separation between the learned models and direct atlas borrowing.}
\label{fig:spatial-top3-hvg}
\end{figure}
